...
double ProcessGenerationNoice::Noise(int i, int x, int y) {
  int n = x + y * 57;
  n = (n << 13) ^ n;
  int a = (this->m_data.seed + x) * 0x7fffffff % 0xffffffff, 
      b = (this->m_data.seed + y) * 0x7fffffff % 0xffffffff, 
      c = (this->m_data.seed + x + y) * 0x7fffffff % 0xffffffff;
  int t = (n * (n * n * a + b) + c) & 0x7fffffff;
  return 1.0 - (double)(t)/0x7fffffff;
}
