...
public:
  Cell(sf::Vector2f pos, float cell_size, sf::Texture &texture,
       unsigned int ID = 0)
...
class Inventory {
public:
  Inventory(sf::Vector2f screen_size, unsigned int rows, unsigned int cols,
            sf::Font &font, float cell_size);
  virtual ~Inventory();
