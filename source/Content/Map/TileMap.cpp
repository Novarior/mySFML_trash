...
void TileMap::initTrees() {
  ...
  for (auto _it = 0; _it < 20; _it++) {
    ...
    shapes.insert(shapes.end(), this->_treeGenerator.get()->internalArray(),
                  this->_treeGenerator.get()->internalArray() +
                      this->_treeGenerator.get()->getSizeArray());
...
void TileMap::generateMap() {
  this->worldSizeGrid =
      sf::Vector2i(_map_dataNoice.mapSizeX, _map_dataNoice.mapSizeY);
  this->worldSizeInt =
      sf::Vector2i(_map_dataNoice.mapSizeX * _map_dataNoice.gridSize,
                   _map_dataNoice.mapSizeY * _map_dataNoice.gridSize);
  this->worldSizeFloat =
      sf::Vector2f(_map_dataNoice.mapSizeX * _map_dataNoice.gridSize,
                   _map_dataNoice.mapSizeY * _map_dataNoice.gridSize);

  float buff_height;
  sf::Color buff;
  this->minimapImage =
      sf::Image(sf::Vector2u(this->worldSizeGrid.x, this->worldSizeGrid.y),
                sf::Color::Red);
    ...
    for (int y = 0; y < _map_dataNoice.mapSizeY; y++) {
      ...
      if (buff_height < 45) { // Ocean
        double depth_intensity =
            100 + buff_height * 1.2; // Интенсивность синего
...
TileMap::TileMap(mmath::noiceData datanoice, ProcessGenerationNoice *noice)
  ...
  this->m_renderArea.offsetX =
      (_map_dataNoice.RenderWindowX / _map_dataNoice.gridSize / 4) + 4;
  this->m_renderArea.offsetY =
      (_map_dataNoice.RenderWindowY / _map_dataNoice.gridSize / 4) + 4;
...
sf::FloatRect TileMap::getGlobalBounds(const unsigned int x,
...
void TileMap::updateWorldBoundsCollision(Entity *entity) { // WORLD BOUNDS
  ...
  } else if (entity->e_getPosition().x + entity->getGlobalBounds().size.x >
             this->worldSizeFloat.x) {
    entity->e_setPosition(this->worldSizeFloat.x -
                              entity->getGlobalBounds().size.x,
  ...
  } else if (entity->e_getPosition().y + entity->getGlobalBounds().size.y >
             this->worldSizeFloat.y) {
    entity->e_setPosition(entity->e_getPosition().x,
                          this->worldSizeFloat.y -
                              entity->getGlobalBounds().size.y);
    entity->getMovement()->stopVelocityY();
  }
}
...
void TileMap::updateTileCollision(Entity *entity, const float &delta_time) {
  ...
  this->m_colisionArea.toX =
      std::min(this->worldSizeGrid.x, entityGridPos.x + 3);
...
void TileMap::updateRenderArea(
    const sf::Vector2i &playerPosition_grid) // update render area
{
  { // update render area
    this->m_renderArea.fromX =
        playerPosition_grid.x - this->m_renderArea.offsetX;
    ...
    this->m_renderArea.fromY =
        playerPosition_grid.y - this->m_renderArea.offsetY;
