...
protected:
  ...
  void createMovementComponent(const float acceleration,
...
public:
  ...
  Entity(entityEnum::entityBehaviorClass enumBehavior =
             entityEnum::entityBehaviorClass::ebc_VOID,
         entityEnum::entityMovementState enumMovementState =
             entityEnum::entityMovementState::ems_VOID,
  ...
  inline const unsigned int &e_getCountEntitys() const {
    return this->count_entitys;
  }
  ...
  inline const bool e_isAlive() const {
    return this->e_attributes->getAttributes().isAlive;
  }

  // Returns the grid size used for the entity.
  // This is typically used for positioning and movement calculations.
  inline virtual const float getGridSizeFloat() const {
  ...
  inline virtual void e_setCollision(const bool isCollision) {
    this->isCollision = isCollision;
  }

  /// @brief Loads a texture from a file and applies it to the entity's sprite.
  /// @param file_path The path to the texture file.
  inline virtual void e_updateTexture(const sf::Texture &_texture) {
    this->m_texture.update(_texture);
  }
  ...
  inline virtual void e_setTexture(sf::Texture *texture) {
  ...
  inline virtual void e_setPosition(const sf::Vector2f pos) {
  ...
  inline virtual void e_setPosition(const float pos_x, const float pos_y) {
  ...
  inline virtual void e_updateMovement(const float &delta_time) {
    this->e_movement->update(delta_time);
  }

  /// @brief Updates the entity's hitbox.
  inline virtual void e_updateHitbox(sf::IntRect rectEntity,
                                     sf::IntRect rectCollision) {
  ...
  /// @param delta_time The time elapsed since the last update.
  virtual void e_updateAnimation(std::string keyNameAnimation,
  ...
  /// @param delta_time The time elapsed since the last update.
  virtual void e_move(const float &dir_x, const float &dir_y,
                      const float &delta_time);
  ...
  inline virtual void e_takeDamage(const int &damage) {
    this->e_attributes->takeDamage(damage);
  }
  ...
  /// or not.
  virtual void e_render(sf::RenderTarget &target,
