...
Player::Player(sf::Vector2f pos)
  ...
  attributes.max_health = 10 + std::floor(5 * (attributes.level - 1)) +
                          (attributes.some_points * 18);
  attributes.max_mana = 5 + std::floor(5 * (attributes.level * 0.25)) +
                        (attributes.some_points / 16) * 10;
  // calculate new regen and max health
  attributes.regeneration_health =
      1 + (attributes.level * 0.15) + (attributes.some_points * 0.0625 / 2);
  attributes.regeneration_mana =
      1 + (attributes.level + (attributes.some_points * 0.01 - 1)) * 0.1;
  // calculate new damage
  attributes.damage =
      1 + (attributes.level * 1.25) + (attributes.some_points * 0.0625 / 2);
  ...
  this->m_radius.setOrigin(
      {this->m_radius.getRadius(), this->m_radius.getRadius()});
...
void Player::e_attack(
    Entity *target,
    const float &delta_time) { // chek if some entity is in range

  if (this->m_radius.getGlobalBounds().findIntersection(
          target->getGlobalBounds())) {
