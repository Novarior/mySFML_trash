...
enum MovamentState { NONe = -1, Idle, Walk, Fall, Shift };
...
public:
  MovementComponent(sf::Sprite &sprite, const float acceleration,
                    const float deceleration, const float maxVelocity);
  ~MovementComponent();
  ...
  float handleVelocity(float velocity, float deceleration, float maxVelocity,
                       float delta_time);
